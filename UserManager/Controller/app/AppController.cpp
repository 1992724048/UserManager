...
namespace controller {
    ...
    auto AppController::create_app(const httplib::Request& req, httplib::Response& res) -> void {
        ...
        try {
            const std::string app_name = json["app_name"];
            const std::string app_ver = json["app_ver"];
            json.clear();
            ...
            if (AppDao::Add(app_name, app_ver)) {
                json["success"] = true;
                json["message"] = "添加成功!";
                res.set_content(json.dump(), "application/json");
            } else {
    ...
    auto AppController::update_app(const httplib::Request& req, httplib::Response& res) -> void {
        ...
        try {
            ...
            if (AppDao::Update(app_name, is_stop) && AppDao::Update(app_name, app_ver)) {
                json["success"] = true;
                json["message"] = "更新成功!";
                res.set_content(json.dump(), "application/json");
            } else {
    ...
    auto AppController::get_ver(const httplib::Request& req, httplib::Response& res) -> void {
        nlohmann::json json = nlohmann::json::parse(req.body);
