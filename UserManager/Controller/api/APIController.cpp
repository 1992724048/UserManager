...
auto APIController::info(const httplib::Request& req, httplib::Response& res) -> void {
    ...
    for (int i = 0; i < num_cores; i++) {
        ...
        if (status == ERROR_SUCCESS) {
            cpuCoreCounters.push_back(core_counter);
        } else {
...
auto APIController::get_all(const httplib::Request& req, httplib::Response& res) -> void {
    ...
    try {
        for (const auto& [fst, snd] : controllers) {
            json["data"][snd.group].push_back({{"group", snd.group}, {"name", fst}, {"url", snd.url_path}, {"note", snd.note}, {"type", snd.request}});
        }
...
auto APIController::get_doc(const httplib::Request& req, httplib::Response& res) -> void {
    apis["APIController::timestamp"] = {
        ...
        .type = httplib::ALL,
        .desc = "API",
        .params = {{.name = "timestamp", .type = "int", .required = false, .desc = "时间戳"}}
    };
    ...
    apis["APIController::ip"] = {
        ...
        .type = httplib::ALL,
        .desc = "API",
        .params = {{.name = "ip", .type = "string", .required = false, .desc = "访问者IP"}, {.name = "port", .type = "int", .required = false, .desc = "访问者端口"}}
    };
    ...
    apis["APIController::info"] = {
        ...
        .desc = "API",
        .params = {
            {.name = "cpu", .type = "string", .required = false, .desc = "CPU使用率"},
            {.name = "memory", .type = "string", .required = false, .desc = "内存使用率"},
            {.name = "disk", .type = "string", .required = false, .desc = "磁盘使用率"},
            {.name = "network", .type = "string", .required = false, .desc = "网络使用率"}
        }
    };
    ...
    apis["APIController::get_all"] = {
        ...
        .type = httplib::COOKIE,
        .desc = "API",
        .params = {{.name = "data", .type = "array", .required = false, .desc = "接口数据"}}
    };
    ...
    apis["APIController::set_api"] = {
        ...
        .type = httplib::COOKIE,
        .desc = "API",
        .params = {{.name = "name", .type = "string", .required = true, .desc = "接口名称"}, {.name = "type", .type = "string", .required = true, .desc = "接口类型"}}
    };
    ...
    apis["APIController::get_doc"] = {
        ...
        .type = httplib::COOKIE,
        .desc = "API",
        .params = {{.name = "data", .type = "object", .required = false, .desc = "接口文档"}}
    };
...
auto APIController::check(const httplib::Request& req, httplib::Response& res) -> void {
    ...
    try {
        for (const auto& [fst, snd] : controllers) {
            if (controllers_default.contains(fst)) {
                ...
                if (type != snd.request && snd.request == httplib::ALL) {
                    json["data"].push_back({{"name", snd.note}, {"type", "API"}, {"note", "权限异常"}, {"suggestion", "建议调整成默认权限"}});
                }
            }
        }

        if (service.f_username.value().size() < 8) {
            json["data"].push_back({{"name", "管理员用户名"}, {"type", "Administrator"}, {"note", "用户名过短"}, {"suggestion", "建议使用至少8位字符的用户名"}});
        }

        if (service.f_password.value().size() < 8) {
            json["data"].push_back({{"name", "管理员密码"}, {"type", "Administrator"}, {"note", "密码过短"}, {"suggestion", "建议使用至少8位字符的密码"}});
        }
        ...
        if (!has_upper) {
            json["data"].push_back({{"name", "管理员密码"}, {"type", "Administrator"}, {"note", "缺少大写字母"}, {"suggestion", "建议至少包含一个A-Z的字符"}});
        }

        if (!has_lower) {
            json["data"].push_back({{"name", "管理员密码"}, {"type", "Administrator"}, {"note", "缺少小写字母"}, {"suggestion", "建议至少包含一个a-z的字符"}});
        }

        if (!has_digit) {
            json["data"].push_back({{"name", "管理员密码"}, {"type", "Administrator"}, {"note", "缺少数字"}, {"suggestion", "建议至少包含一个0-9的数字"}});
        }

        if (!has_special) {
            json["data"].push_back({{"name", "管理员密码"}, {"type", "Administrator"}, {"note", "缺少特殊符号"}, {"suggestion", "建议至少包含一个特殊字符（如!@#$%^&*_+-等）"}});
        }
