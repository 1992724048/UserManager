...
namespace controller {
    ...
    public:
        METHOD_LIST_BEGIN
            ...
            METHOD_ADD(UserController::user_ban, "/users/user_ban", httplib::POST, httplib::COOKIE, "用户", "封禁用户");
            ...
            METHOD_ADD(UserController::send_email, "/users/send_email", httplib::POST, httplib::ALL, "用户", "邮箱验证码");
        METHOD_LIST_END
    private:
        // 一定要写构造方法，不然会被优化掉
        UserController();
        ...
        static auto send_email(const httplib::Request& req, httplib::Response& res) -> void;
        static auto user_ban(const httplib::Request& req, httplib::Response& res) -> void;

        static auto clear_captcha() -> void;
        static auto clear_key_heartbeat() -> void;

        inline static util::SafeMap<std::string, std::pair<std::chrono::steady_clock::time_point, std::string>> captchas;
        inline static util::SafeMap<std::string, std::pair<std::chrono::steady_clock::time_point, std::string>> email_captchas;
