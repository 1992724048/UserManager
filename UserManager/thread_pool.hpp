...
namespace tp {
	...
	*/
	...
	public:
		 ...
		 */
		auto max_threads() const -> int { return arena_.max_concurrency(); }
		...
		auto push(F&& f, Args&&... args) -> std::future<std::invoke_result_t<F, Args...>> {
			...
			auto task_ptr = std::make_shared<std::packaged_task<ResultType()>>(
				[func = std::forward<F>(f), args_tuple = std::make_tuple(std::forward<Args>(args)...)]()mutable {
					return std::apply([&func]<typename... T>(T&&... args) -> ResultType { return std::invoke(func, std::forward<T>(args)...); },
						std::move(args_tuple));
				});
		...
		auto wait() -> void {
			...
			cv_.wait(lock, [this] { return task_count_.load(std::memory_order_relaxed) == 0; });
		}
		...
		auto wait_for(const std::chrono::duration<Rep, Period>& duration) -> bool {
			std::unique_lock lock(mtx_);
			return cv_.wait_for(lock, duration, [this] { return task_count_.load(std::memory_order_relaxed) == 0; });
		}
		...
		auto push_loop(Iterator begin,
			Iterator end,
			F&& f) -> std::vector<std::future<std::invoke_result_t<F, typename std::iterator_traits<Iterator>::value_type>>> {
