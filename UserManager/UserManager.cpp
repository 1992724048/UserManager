...
static auto WINAPI UnhandledExceptionFilter_(const PEXCEPTION_POINTERS p_exception_info) -> LONG {
	...
	const PSYMBOL_INFO p_symbol = reinterpret_cast<PSYMBOL_INFO>(symbol_info);
	...
	const std::string dump_path = (util::AppPath() / "dump.dmp").string();
	...
	if (h_file.get() != INVALID_HANDLE_VALUE) {
		...
		constexpr MINIDUMP_TYPE dump_type = static_cast<MINIDUMP_TYPE>(
			MiniDumpWithFullMemory | // 包含所有可访问进程内存
...
auto main() -> int {
	...
	config::Initialize(util::AppPath() / "config.json");
	Logger::PrepareFileLogging(util::AppPath() / "logs");

	try { return Service::Instance().Run(); } catch (std::exception& exception) {
