...
#include "handlers/functoreventhandler.hpp"
#include "handlers/methodeventhandler.hpp"
#include "joins/eventjoinwrapper.hpp"
#include "joins/handlereventjoin.h"
#include "joins/handlereventjoin.hpp"
...
public:
    ...
    auto operator+=(TSome&& some) -> events::EventJoin {
...
protected:
    ...
    virtual auto isHandlerAdded(const TMyEventHandlerPtr& eventHandler) const -> bool = 0;
    virtual auto addHandler(TMyEventHandlerPtr eventHandler) -> bool = 0;
    virtual auto removeHandler(TMyEventHandlerPtr eventHandler) -> bool = 0;
...
struct EventCore {
    ...
    EventCore(const EventCore& other) : handlers(other.handlers) {
    }

    EventCore(EventCore&& other) : handlers(std::move(handlers)) {
    }

    auto operator=(const EventCore& other) -> EventCore& {
        handlers = other.handlers;
        return *this;
    }

    auto operator=(EventCore&& other) -> EventCore& {
...
public:
    HandlerRunner(TMyEventCore& eventCore) : currentIt(), wasRemoving(false), m_eventCore(eventCore) {
    }

    auto run(TParams... params) -> void {
...
public:
    ...
    virtual auto operator()(TParams... params) -> void {
...
protected:
    auto isHandlerAdded(const TMyEventHandlerPtr& eventHandler) const -> bool override {
        std::shared_lock<std::shared_mutex> _coreMutexLock(m_core.coreMutex);

        return (findEventHandler(eventHandler) != m_core.handlers.end());
    }

    auto addHandler(TMyEventHandlerPtr eventHandler) -> bool override {
    ...
    auto removeHandler(TMyEventHandlerPtr eventHandler) -> bool override {
        std::unique_lock<std::shared_mutex> _coreMutexLock(m_core.coreMutex);
...
private:
    auto findEventHandler(const TMyEventHandlerPtr& eventHandler) const noexcept -> TMyEventHandlerIt {
        return std::ranges::find_if(m_core.handlers,
                                    [&eventHandler](const TMyEventHandlerPtr& oneHandler) {
...
public:
    auto operator()(TParams... params) -> bool {
        bool canceled = false;
